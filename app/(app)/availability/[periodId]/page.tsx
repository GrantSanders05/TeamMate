...
export default function AvailabilityPage() {
  ...
  async function loadData() {
    const [periodRes, shiftsRes, responsesRes] = await Promise.all([
      ...
      supabase.from('availability_responses').select('*').eq('employee_id', member!.user_id).in(
        'shift_id',
    ...
    for (const r of (responsesRes.data || [])) {
      existing[r.shift_id] = r.status as AvailStatus
    }
  ...
  function toggleShift(shiftId: string, status: AvailStatus) {
    setResponses(r => ({
      ...r,
      [shiftId]: r[shiftId] === status ? null : status,
    }))
  }
  ...
  async function handleSubmit() {
    ...
    if (nulledIds.length > 0) {
      await supabase.from('availability_responses')
        .delete()
        .in('shift_id', nulledIds)
        .eq('employee_id', member.user_id)
    }

    if (upserts.length > 0) {
      const { error } = await supabase.from('availability_responses').upsert(upserts, { onConflict: 'shift_id,employee_id' })
      if (error) {
    ...
    toast({ title: 'Availability saved!', description: 'Your responses have been recorded.' })
    setSaving(false)
  }
    ...
    return (
      <div className="max-w-lg mx-auto text-center py-20">
        ...
        <p className="text-slate-500">
          {period.status === 'published' ? 'This schedule has been published.' : 'The manager has closed availability for this period.'}
        </p>
      </div>
    )
  }
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      >
        {organization?.logo_url && (
          <img src={organization.logo_url} alt={organization.name} className="h-8 mb-3 rounded" />
        )}
        <div className="font-semibold text-lg">{period.name}</div>
        <div className="text-sm opacity-80 mt-0.5">
          {formatDate(period.start_date)} — {formatDate(period.end_date)} · {organization?.name}
        </div>
        <div className="mt-3 text-sm opacity-90">
          {respondedCount} of {totalShifts} shifts responded
        </div>
      </div>

      {/* Instructions */}
      <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-sm text-blue-700">
        Mark each shift as <strong>Available</strong> or <strong>Unavailable</strong>. You can use the day buttons to mark all shifts in a day at once.
      </div>
            ...
            <div key={day} className="bg-white border border-slate-200 rounded-xl overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 bg-slate-50 border-b border-slate-100">
                ...
                <div className="flex gap-1.5">
                  <button
                    onClick={() => markAllDay(day, 'available')}
                    className="text-xs px-2 py-1 rounded-md bg-green-100 text-green-700 hover:bg-green-200 font-medium"
                  >
                    All available
                  </button>
                  <button
                    onClick={() => markAllDay(day, 'unavailable')}
                    className="text-xs px-2 py-1 rounded-md bg-red-100 text-red-700 hover:bg-red-200 font-medium"
                  >
                    All unavailable
                  </button>
                </div>
                  ...
                  return (
                    <div key={shift.id} className="flex items-center justify-between px-4 py-3.5">
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() => toggleShift(shift.id, 'available')}
                          className={cn(
                        ...
                        <button
                          onClick={() => toggleShift(shift.id, 'unavailable')}
                          className={cn(
                            'flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-all',
                            status === 'unavailable'
