...
export default function ScheduleBuilderPage() {
  ...
  async function loadAll() {
    ...
    const { data: membersData } = await supabase
      .from('organization_members').select('*, profiles(*)')
      .eq('organization_id', organization!.id).eq('is_active', true)
    ...
    if (shiftIds.length > 0) {
      const [availRes, assignRes] = await Promise.all([
        supabase.from('availability_responses').select('*').in('shift_id', shiftIds),
        supabase.from('shift_assignments').select('*, profiles(*)').in('shift_id', shiftIds).neq('status', 'dropped'),
      ])
  ...
  async function updateStatus(newStatus: string) {
    const { error } = await supabase.from('scheduling_periods').update({
      status: newStatus,
  ...
  async function addShift(shiftData: any) {
    const { data, error } = await supabase.from('shifts')
      .insert({ ...shiftData, scheduling_period_id: periodId }).select().single()
    if (error) { toast({ title: 'Error', description: error.message, variant: 'destructive' }); return }
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col lg:flex-row lg:items-center gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          {isManager && period.status === 'draft' && (
            <Button size="sm" onClick={() => updateStatus('collecting')} className="bg-blue-600 hover:bg-blue-700">Open for Availability</Button>
          )}
          {isManager && period.status === 'collecting' && (
            <>
              <Button size="sm" variant="outline" onClick={async () => { await copyToClipboard(availLink); setLinkCopied(true); setTimeout(() => setLinkCopied(false), 2000) }}>
                {linkCopied ? <Check className="w-4 h-4 mr-1.5 text-green-600" /> : <Copy className="w-4 h-4 mr-1.5" />}
                Copy availability link
              </Button>
              <Button size="sm" onClick={() => updateStatus('scheduling')} className="bg-amber-600 hover:bg-amber-700">Close Availability</Button>
            </>
          )}
          {isManager && period.status === 'scheduling' && (
            <>
              ...
              <Button size="sm" onClick={() => updateStatus('published')} className="bg-green-600 hover:bg-green-700">Publish Schedule</Button>
            </>
          )}
          {isManager && period.status === 'published' && (
            <Button size="sm" variant="outline" onClick={() => updateStatus('scheduling')}>Unpublish for edits</Button>
          )}
          {isManager && period.status === 'published' && (
      ...
      <div className="flex gap-5 min-h-[600px]">
        {isManager && <EmployeePanel members={members} assignments={assignments} shifts={shifts} />}
        <div className="flex-1 min-w-0">
          {weeks.length > 1 && (
            <div className="flex items-center justify-between mb-3">
              <Button variant="ghost" size="sm" onClick={() => setCurrentWeek(w => Math.max(0, w - 1))} disabled={currentWeek === 0}>
                <ChevronLeft className="w-4 h-4 mr-1" /> Prev week
              </Button>
              <span className="text-sm text-slate-500">Week {currentWeek + 1} of {weeks.length}</span>
              <Button variant="ghost" size="sm" onClick={() => setCurrentWeek(w => Math.min(weeks.length - 1, w + 1))} disabled={currentWeek === weeks.length - 1}>
                Next week <ChevronRight className="w-4 h-4 ml-1" />
              </Button>
            </div>
          )}
          <WeekView
            days={currentDays} shifts={shifts} assignments={assignments}
            availability={availability} members={members} canEdit={canEdit}
            onShiftClick={setSelectedShift} onAddShift={setAddShiftDay}
          />
        </div>
      </div>
      ...
      {selectedShift && (
        <ShiftDetailModal
          shift={selectedShift} members={members}
          ...
          canEdit={canEdit} onClose={() => setSelectedShift(null)}
      ...
      {addShiftDay && (
        <AddShiftForm date={addShiftDay} shiftTypes={shiftTypes} onAdd={addShift} onClose={() => setAddShiftDay(null)} />
      )}
    </div>
  )
}
